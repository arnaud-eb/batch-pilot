...
export interface ProductFilters {
  /** Inclusive lower bound on the product's minimum variant price. */
  priceMin?: number;
  /** Inclusive upper bound on the product's minimum variant price. */
  priceMax?: number;
  ...
  collectionId?: string;
  /** Hard cap on results returned. Defaults to 250. */
  limit?: number;
}
...
function buildQueryString(filters: ProductFilters): string {
  ...
  if (filters.tag) clauses.push(`tag:'${filters.tag.replace(/'/g, "\\'")}'`);
  if (filters.collectionId) {
    const numericId = filters.collectionId.split("/").pop();
    if (numericId) clauses.push(`collection_id:${numericId}`);
  }
